...
import org.apache.fineract.organisation.staff.data.LoanAccountSummary;
...
public class BulkLoansReadPlatformServiceImpl implements BulkLoansReadPlatformService {
    ...
    public StaffAccountSummaryCollectionData retrieveLoanOfficerAccountSummary(final Long loanOfficerId) {
        ...
        final List<LoanAccountSummary> clientSummaryList = this.jdbcTemplate.query(clientSql,
                staffClientMapper, new Object[] { loanOfficerId, ClientStatus.ACTIVE.getValue() });

        for (final LoanAccountSummary clientSummary : clientSummaryList) {
        ...
        final List<LoanAccountSummary> groupSummaryList = this.jdbcTemplate.query(groupSql,
                staffGroupMapper, new Object[] { loanOfficerId, GroupingTypeStatus.ACTIVE.getValue() });

        for (final LoanAccountSummary groupSummary : groupSummaryList) {
        ...
        return StaffAccountSummaryCollectionData.builder()
            .clients(clientSummaryList)
            .groups(groupSummaryList)
            .build();
    }

    private static final class StaffClientMapper implements RowMapper<LoanAccountSummary> {
        ...
        @Override
        public LoanAccountSummary mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
                throws SQLException {
    ...
    private static final class StaffGroupMapper implements RowMapper<LoanAccountSummary> {
