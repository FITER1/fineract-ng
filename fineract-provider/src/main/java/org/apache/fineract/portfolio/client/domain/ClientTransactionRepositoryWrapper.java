...
public class ClientTransactionRepositoryWrapper {
    ...
    public ClientTransaction findOneWithNotFoundDetection(final Long clientId, final Long transactionId) {
        ...
        if (clientTransaction.getClient()!=null && clientTransaction.getClient().getId()!=null && !clientTransaction.getClient().getId().equals(clientId)) { throw new ClientTransactionNotFoundException(clientId, transactionId); }
