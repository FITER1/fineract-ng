...
public class CodeWritePlatformServiceJpaRepositoryImpl implements CodeWritePlatformService {
    ...
    @Transactional
    @Override
    @CacheEvict(value = "codes", key = "@fineractProperties.getTenantId().concat('cv')")
    public CommandProcessingResult createCode(final JsonCommand command) {
        ...
        try {
            ...
            return CommandProcessingResult.builder().commandId(command.commandId()).resourceId(code.getId()).build();
    ...
    public CommandProcessingResult deleteCode(final Long codeId) {
        ...
        return CommandProcessingResult.builder().resourceId(codeId).build();
    }
